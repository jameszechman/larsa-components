...
class LarsaAlert extends LitElement {
	 ...
	 */
	@property({ type: String }) status?: 'warning' | 'error' | 'success' | 'info';
	/**
	...
	private handleAlertDismissible() {
		this.dismissed = true;
		this.dispatchEvent(new CustomEvent('dismissed', { bubbles: false }));
	}
	...
	override render(): TemplateResult<1> | undefined {
		...
		if (!this.dismissed)
            ...
            >
                ...
                <slot>This page is not editable. To edit it you must migrate it
                    to a new or existing world.
                </slot>
                ${
									this.dismissible
										? html`
